...
<template>
  <div>
    <div>
      <p v-if="highDeviation" class="text-2xs text-qwerty-red mb-4">
        {{ $t('trade.execution_price_far_away_from_last_traded_price') }}
      </p>
      <p v-if="notEnoughInjForGas" class="text-2xs text-qwerty-red mb-4">
        {{ $t('insufficientGas.tradingFormNote') }}
        <a
          :href="faucetUrl"
          target="_blank"
          class="flex items-center text-qwerty-primary"
        >
          <span class="mr-1">
