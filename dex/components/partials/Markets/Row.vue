...
<template>
  <div
    class="grid grid-cols-3 sm:grid-cols-10 3md:grid-cols-12 text-qwerty-white gap-4 text-sm px-4 py-5 mb-1 items-center hover:bg-qwerty-shade3 border-b border-qwerty-shade3 last:border-0"
    :data-cy="`markets-table-row-${market.ticker}`"
  >
    <span class="text-sm col-span-2 sm:col-span-3 flex items-center gap-4">
      <div
        class="3md:hidden text-qwerty-primary mr-3 cursor-pointer"
      ...
      <NuxtLink :to="marketRoute" class="w-full cursor-pointer">
        ...
        >
          ...
          <div class="flex flex-col w-full overflow-hidden">
            ...
            <span class="text-qwerty-white text-xs hidden md:block">
              {{ market.baseToken.name }}
            </span>
            <span
              class="text-qwerty-white text-xs sm:hidden tracking-wide mt-1"
            >
    ...
    <div class="sm:hidden flex flex-col items-end font-mono">
      ...
      <div v-if="!change.isNaN()" class="mt-1">
        <span :class="change.gte(0) ? 'text-qwerty-green' : 'text-qwerty-red'">
          {{ changeToFormat }}%
        </span>
      </div>
    </div>
    ...
    <span class="hidden 3md:flex col-span-2 items-center justify-end">
      <NuxtLink
        :to="marketRoute"
        class="text-qwerty-white hover:text-qwerty-primary cursor-pointer"
      ...
      <div
        class="text-qwerty-white w-6 h-6 flex items-center justify-center rounded-full ml-6 cursor-pointer hover:bg-qwerty-primary hover:bg-opacity-10"
      ...
      >
        <BaseIcon
          v-if="isFavorite"
          name="star"
          class="min-w-5 w-5 h-5 text-qwerty-primary"
