...
<template>
  <tr
    v-if="market"
    class="border-b border-gray-600 last-of-type:border-b-transparent hover:bg-qwerty-shade3 bg-transparent px-4 py-0 overflow-hidden h-14 gap-2 transition-all cursor-pointer"
  ...
  >
    <td class="pl-4">
      <div class="col-span-1 flex justify-start items-center gap-2">
        ...
        <span
          class="text-qwerty-white font-bold tracking-wide text-xs lg:text-sm uppercase"
        >
          {{ position.ticker }}
        </span>
      </div>
    ...
    <td>
      <div class="col-span-1 flex flex-col text-right">
        ...
        <span
          ...
          class="font-mono text-xs lg:text-sm text-qwerty-white"
        >
          {{ markPriceToFormat }}
        </span>
        <span v-else class="font-mono text-xs lg:text-sm text-qwerty-white">
          {{ HIDDEN_BALANCE_DISPLAY }}
        </span>
      </div>
    </td>
    ...
    <td>
      <div class="col-span-1 flex gap-1 justify-end">
        <div v-if="hideBalances" class="flex items-center gap-1">
          ...
          <span class="text-xs lg:text-sm text-qwerty-white uppercase">
            {{ market.quoteToken.symbol }}
          </span>
        </div>
    ...
    <td>
      <div class="col-span-1 text-right">
        <span
          v-if="isBinaryOptions"
          class="text-qwerty-white text-xs lg:text-sm font-mono"
        >
          &mdash;
        </span>
        <span
        ...
        <span
          ...
          class="flex items-center justify-end text-qwerty-white text-xs lg:text-sm font-mono"
          data-cy="open-position-leverage-table-data"
        >
          {{ effectiveLeverage.toFormat(2) }}
          <span class="text-qwerty-white text-xs lg:text-sm font-mono">
            &times;
          </span>
        </span>
        <span
    ...
    <td class="pr-4">
      <div class="col-span-1 flex items-center justify-end gap-4">
        ...
        >
          <div
            class="flex items-center justify-center rounded-full bg-opacity-10 w-6 h-6 hover:bg-opacity-10 bg-qwerty-red hover:bg-red-600 text-qwerty-red hover:text-qwerty-red"
          >
