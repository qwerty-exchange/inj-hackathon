  ...
  <div class="flex flex-col bg-gray-850 rounded-lg p-6 h-full">
    <div class="flex justify-start gap-6 lg:gap-8">
      <div class="flex flex-col border-r border-gray-500 pr-6 lg:pr-8">
        <span
          class="text-qwerty-white uppercase tracking-wide text-xs mb-2 font-semibold whitespace-nowrap"
        >
          {{ $t('fee_discounts.my_tier') }}
        </span>
        <span
          class="uppercase text-xl lg:text-2xl font-bold tracking-normal text-qwerty-white"
        >
          #{{ tierLevel }}
        </span>
      </div>
      <div class="flex flex-col">
        ...
        <span
          class="uppercase text-xs lg:text-base text-qwerty-white font-bold tracking-widest whitespace-nowrap"
        >
          <b
            class="text-xl lg:text-2xl font-bold text-qwerty-white tracking-normal font-mono"
          >
            {{ makerFeeDiscount }}%
          </b>
          {{ $t('fee_discounts.off') }}
    ...
    <div class="mt-4">
      <span v-if="lastUpdateTimestamp" class="text-xs text-qwerty-white">
        {{ $t('fee_discounts.update_daily') }}.
        {{ $t('fee_discounts.last_updated_at') }} {{ lastUpdateTimestamp }}
      </span>
      <span v-else class="text-xs text-qwerty-white">&mdash;</span>
    </div>
  </div>
</template>
