  ...
  <BaseDropdown class="inline-block" placement="top-end">
    <template #default="{ shown }">
      ...
      >
        <span
          v-if="selectedOption"
          class="font-semibold text-qwerty-white text-sm"
        >
          {{ selectedOption }}
        </span>
      ...
      <div class="p-2 bg-gray-850 shadow-sm">
        ...
        >
          <template #default="{ active }">
            <div
              class="group flex cursor-pointer items-center justify-between rounded hover:bg-qwerty-shade3 p-2 gap-2 text-sm"
            >
              <span
                class="font-semibold group-hover:text-qwerty-primary text-sm"
                :class="{
                  'text-qwerty-white': !active,
                  'text-qwerty-primary': active
                }"
              >
                {{ item }}
              </span>
