...
const showBalance = computed(() => !props.hideBalance)
...
const {
  ...
  dynamicRule: computed(() => {
    ...
    return showBalance.value
      ? `insufficientBalance:${maxBalanceToFixed.value}|required`
      : `required`
  })
})
...
<template>
  <div
    class="bg-qwerty-background rounded-xl py-4"
    :class="{
      'border-red-500 border': amountErrors.length > 0
    }"
  >
    <div
      class="text-sm font-semibold text-qwerty-white flex items-center justify-between px-4 mb-2"
    >
      <slot />
      ...
      <div v-if="selectedToken" class="text-right flex items-center gap-2">
        <span
          ...
          class="cursor-pointer text-qwerty-primary hover:text-opacity-80"
          @click="handleMax"
        >
          {{ $t('trade.max') }}:
        </span>
        <p v-if="showBalance">
          {{ maxBalanceToString }} {{ selectedToken.token.symbol }}
        </p>
      </div>
    </div>
    ...
    <div class="flex items-center justify-between gap-2 px-4">
      <div class="flex flex-wrap items-center gap-1 text-sm whitespace-nowrap">
        <span v-if="amountErrors.length > 0" class="text-qwerty-red capitalize">
          {{ amountErrors[0] }}
        </span>
      </div>
    </div>
...
.dropdown.v-popper--theme-dropdown .v-popper__inner {
  @apply bg-qwerty-shade2 border-blue-300 shadow-sm;
}
</style>
